...
| `system-soak` | weekly or manual | GitHub-hosted Docker | 1 | Five-hour namespace/storage churn with RSS, descriptor, and disk-growth regression analysis. |
| `system-wan-optional` | manual | non-owning remote | 2 | Optional Tailscale and public direct-WAN diagnostics when suitable infrastructure exists. |
| `firecracker-host-gated-optional` | manual | host KVM | 1 | Optional real Firecracker execution on an eligible disposable host. |
...
Pull-request smoke uses a fixed seed. Daily jobs rotate from the UTC day; soak jobs rotate from the UTC week. A manual dispatch may supply an exact seed. The selected seed appears in the artifact name, `run.json`, event stream, and generated `reproduce.sh`. Soak reproduction also records its duration and exact hosted-runner kernel.
...
- Soak and optional WAN/KVM diagnostic artifacts: 60 days.
...
`SOAK-001` requires `--duration-seconds` and `--expected-kernel`. CI uses a five-hour default on `ubuntu-latest`, records the provisioned runner kernel, and optionally rejects a manually requested kernel mismatch. It repeatedly writes and reads bounded content, advances namespace metadata, restarts followers, and archives raw samples plus least-squares RSS, descriptor, and storage-growth analysis. The agent image digest and recorded kernel are release evidence.

`WAN-001` and `KVM-001` remain available as manual, non-gating diagnostics. They are not required by the release policy because ordinary GitHub-hosted runners cannot supply the external multi-site topology or Firecracker/KVM host contract.

A release owner supplies successful smoke, functional, chaos, and soak workflow run IDs to `release-qualification`. The workflow verifies every source run is successful and has the exact release commit, downloads its archives, and applies `ci/qualification-policy.json`. Missing scenarios, wrong backends, absent evidence, missing Docker digests, unfinalized manifests, unsafe paths, or commit mismatches produce an incomplete JSON and Markdown report and fail the gate.

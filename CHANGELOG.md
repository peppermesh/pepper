...
- Added an opt-in S3-compatible gateway with Signature Version 4 header, presigned URL, and browser POST authentication; path-style and virtual-hosted routing; object, listing, range, copy, conditional, multi-delete, checksum, and AWS streaming-signature support; durable multipart upload and part copy; bucket tagging, CORS, and incomplete-multipart lifecycle controls; and a three-node Docker AWS CLI contract.
...
- Changed release qualification to require only GitHub-hosted smoke, functional, chaos, and five-hour soak evidence; WAN and Firecracker/KVM scenarios remain available as optional manual diagnostics; and restored system-test image compatibility with Docker's legacy build API.
